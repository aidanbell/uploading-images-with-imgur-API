...
### `express-busboy`
This package is a body parser, and we use it to extend express' built in body parser, because it solves a couple problems for us. In this context, it's main function is to help us attach the image to the `req` object, so that we can use it in our back-end. 

Once we have it installed, we'll need to require it in our `server.js`, and make sure that we're telling our server to use it. 
...
```
And that's it for `express-busboy`! On to...

### `fs`
This short and sweet package gives us better access and control over our file system, and in this context, will be used to pull binary data of our image, and encode it to base64 (the format that the Imgur API is expecting to receive).
...
2. It will have to hit a controller method to send the image

That being said, our route can look something like this:
...
Now lets get to our actual controller function called `upload`. Lets first define it, and then have a look at what `express-busboy` is doing for us. 
...
{
  image: {
    uuid: '58d8469a-9796-425b-8a63-45962bd9360e',
    field: 'image',
    file: '/tmp/express-busboy/58d8469a-9796-425b-8a63-45962bd9360e/image/image.png',
    filename: 'image.png',
    encoding: '7bit',
    mimetype: 'image/png',
    truncated: false,
...
```
So let's start on that `objects` variable.
